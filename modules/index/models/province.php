    ...
    public function submit(Request $request)
    {
        ...
        if ($request->initSession() && $request->isSafe()) {
            if (Login::isAdmin()) {
                // ค่าที่ส่งมา
                $save = array();
                $details = $request->post('province', array())->topic();
                ...
                if (empty($ret)) {
                    ...
                    // province.json
                    $f = @fopen(ROOT_PATH.'Province/province.json', 'wb');
                    if ($f) {
                        fwrite($f, json_encode($save, JSON_UNESCAPED_UNICODE));
                        fclose($f);
                        // คืนค่า
                        $ret['alert'] = Language::get('Saved successfully');
                    } else {
                        // คืนค่า
                        $ret['alert'] = sprintf(Language::get('File %s cannot be created or is read-only.'), 'province.json');
                    }
                    $ret['location'] = 'reload';
                    // เคลียร์
                    $request->removeToken();
